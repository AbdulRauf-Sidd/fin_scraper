...
async def extract_files_from_page(page):
    ...
    try:
        # Select all year tabs (from 2024 to 2019)
        year_tabs = await page.query_selector_all(".cmp-tabs__tablist .cmp-tabs__tab a")

        for year_tab in year_tabs:
            year_text = await year_tab.inner_text()
            ...
            for block in event_blocks:
                ...
                # Extract event date
                date_element = await block.query_selector(".cmp-text p b")
                if date_element:
            ...
            # Append structured event data if valid
            if event_name and event_date and file_links:
